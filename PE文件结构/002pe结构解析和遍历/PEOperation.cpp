...
WIN_CERTIFICATE* GetCertificate(IN BYTE *pFileBuf)
{
	IMAGE_DATA_DIRECTORY* pDirectory = GetDirectory(pFileBuf);
	if (pDirectory[IMAGE_DIRECTORY_ENTRY_SECURITY].VirtualAddress)
	{
		return (WIN_CERTIFICATE*)(pFileBuf + (SIZE_T)pDirectory[IMAGE_DIRECTORY_ENTRY_SECURITY].VirtualAddress);
	}
	{
		return NULL;
	}
