...
#endif
{
	...
	msgMap.insert({ 0x1A, _T("WM_WININICHANGE") });
	msgMap.insert({ 0x1A, _T("WM_SETTINGCHANGE") });
	...
	msgMap.insert({ 0x100, _T("WM_KEYFIRST") });
	msgMap.insert({ 0x100, _T("WM_KEYDOWN") });
...
void PrintWindowsMsgInfo(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	{
		PrintDbgInfo(_T("hwnd:0x%X:uMsg:0x%X:wParam:0x%X:lParam:0x%X"), hwnd, uMsg, wParam, lParam);
	}
	else
	{
		PrintDbgInfo(_T("hwnd:0x%X:uMsg:%s:wParam:0x%X:lParam:0x%X"), hwnd, msgMap[uMsg].c_str(), wParam, lParam);
	}
}
